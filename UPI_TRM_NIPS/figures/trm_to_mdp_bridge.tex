...
\node[font=\bfseries\small] at (4.6, 2.85) {Plan-space MDP view};
...
U_n(s_t) &= V_\psi(z_t^{(n)},x)
\end{aligned}$\par
}
};

\node[rightpanel] (mdp) at (4.6, -1.85) {%
\textbf{Plan-space MDP}\\[-1pt]
State: $s_t=(x,y_t)$\\
Action: edit $a_t$\\
Transition: $y_{t+1}=\mathrm{edit}(y_t,a_t;x)$\\
Reward: checker score $c(x,y_t)$
};

\draw[flow] (eval.south) -- node[right, note] {policy uses\\$U_n$} (mdp.north);
