\begin{table}[t]
\centering
\caption{Projection is the primary stabilizer. $R{=}10$ projection dramatically improves stability metrics compared to \emph{proj.\ off} ($\Pi_R=\mathrm{Id}$).}
\label{tab:exp2_projection_stabilizer}
\begin{tabular}{lccccc}
\toprule
Target $L_z$ & \multicolumn{2}{c}{$\Delta V$ (n=2$\to$16)} & \multicolumn{2}{c}{Argmax Agree (\%)} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
 & $R{=}10$ & \emph{proj.\ off} & $R{=}10$ & \emph{proj.\ off} \\
\midrule
