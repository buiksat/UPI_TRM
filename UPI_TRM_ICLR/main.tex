...
\maketitle
\begin{abstract}
We study fixed-snapshot error bounds for a finite-depth recurrent value evaluator inside policy improvement. For finite reference depths $m>n$, value error is controlled by the depth discrepancy $\|U_n-U_m\|$ and a Bellman residual at depth $m$; a recurrent path bounds the first term without assuming a latent fixed point. For an edit budget $H$ and clock $h\ge1$, a stagewise analysis replaces the infinite-horizon residual factor by $(1-\gamma^{K\lceil h/K\rceil})/(1-\gamma^K)$; at $h=0$ the error and residual sum are zero. Persistent latents use the clock-complete augmented state $(x,y,z,h)$ and one shared frozen recurrent transition map. We also give finite-horizon conservative-policy-improvement bounds under exact statewise centering and exact probability-space mixing, plus a deployment perturbation bound under a uniform total-variation or KL condition. These are conditional, fixed-snapshot results. Empirical evaluation is deferred.
\end{abstract}
...
Fix one discounted MDP $(\mathcal S,\mathcal A,P,r,\rho,\gamma)$ with
$\gamma\in[0,1)$, an integer $K\ge1$, and the fixed policy pair
$(\pi,\pi_{\mathrm{cand}})$. Let the finite or countable set
$\mathcal C:=\mathcal R_{\pi,\pi_{\mathrm{cand}}}^{\mathrm{adv}}
\cup\{s_{\mathrm{abs}}\}$ contain the support reachable from $\rho$ under
$\pi$ and every one-step successor required by an action supported by either
policy. Require the supported action sets of both policies on $\mathcal C$ to
be finite or countable and $\mathcal C$ to be invariant under subsequent
$\pi$ transitions almost surely. The atom $s_{\mathrm{abs}}$ is the common
absorbing self-loop with reward $(\gamma-1)C_{\max}$, so its
policy-independent value is $b:=V^\pi(s_{\mathrm{abs}})=-C_{\max}$. Fix one
frozen parameter snapshot,
one measurable recurrent map shared by every depth, a common measurable
initialization $z^{(0)}(s)=z_{\mathrm{init}}(x,y)$ that ignores the clock, and
one latent norm on $\mathcal Z\subseteq\mathbb R^d$. Let
$V_\psi:\mathcal Z\times\mathcal X\times\mathcal Y\to\mathbb R$ be a
measurable scalar value head. For every $j$, define $z^{(j)}$ by iterating
that same map from that same initialization and set
$U_j(s)=V_\psi(z^{(j)}(s),x,y)$, with the common boundary
$U_j(s_{\mathrm{abs}})=b$. Assume rewards and $U_j$,
$j\in\{n,\ldots,m\}$, are bounded on $\mathcal C$, and
$\mathcal T_K^\pi$ maps bounded real functions on $\mathcal C$ to bounded
real functions on $\mathcal C$. Let $V^\pi$ be the bounded fixed point
$V^\pi=\mathcal T_K^\pi V^\pi$ on this space. All displayed norms are ordinary sup norms
on $\mathcal C$. Then, for any finite integers $0\le n<m$,
\begin{align}
...
\end{align}
If, in addition, the scalar head is $L_V$-Lipschitz in its latent argument uniformly over relevant $(x,y)$ while $(x,y)$ is held fixed, then
\begin{align}
...
The theorem is a residual bound plus a telescoping path bound; we make no novelty claim for the triangle inequality itself. Its role is to expose a finite reference $m$ whose error is certifiable when the displayed uniform discrepancy and residual are available: the recurrence may perform useful transient computation and need not converge before, at, or after $m$. The same frozen policy $\pi$ appears in $V^\pi$ and $\mathcal T_K^\pi$, and the latter bootstraps from $U_m$ itself.
...
Fix one finite or countable discounted MDP, one policy pair
$(\pi,\pi_{\mathrm{cand}})$, $\gamma\in[0,1)$, and $K\ge1$. Let
$\mathcal C:=\mathcal{R}_{\pi,\pi_{\mathrm{cand}}}^{\mathrm{adv}}
\cup\{s_{\mathrm{abs}}\}$ contain the initial support and every one-step
successor required by an action supported by either policy, and require it to
be forward invariant under subsequent $\pi$ transitions almost surely. Assume
the supported action sets are finite or countable and rewards are bounded on
$\mathcal C$. Let $B(\mathcal C)$ be the bounded real functions with the
ordinary sup norm on $\mathcal C$, require
$\mathcal T_K^\pi:B(\mathcal C)\to B(\mathcal C)$, and let
$V^\pi\in B(\mathcal C)$ satisfy
$V^\pi=\mathcal T_K^\pi V^\pi$. Use one common absorbing boundary
$U_n(s_{\mathrm{abs}})=V^\pi(s_{\mathrm{abs}})=b$, and take
$U_n\in B(\mathcal C)$. Define
\begin{equation}
...
This certificate follows from Bellman contraction alone and applies to finite
MDPs and to finite or countable deterministic edit environments satisfying
the displayed bounded-function, closure, Bellman-domain, and common-boundary
assumptions. Pointwise computability or determinism alone is insufficient. For
example, on the countable zero-reward chain $s_i\mapsto s_{i+1}$ with one
...
Let $\bar{\mathcal C}$ be the preceding finite or countable augmented closure,
including the shared absorber, and let $B(\bar{\mathcal C})$ carry its ordinary
sup norm. Under the preceding frozen-MDP, measurability, bounded-reward,
common-boundary, and closure assumptions, require
$\widetilde U_n,V^{\bar\pi}\in B(\bar{\mathcal C})$,
$\bar{\mathcal T}^{\bar\pi}_K:B(\bar{\mathcal C})
\to B(\bar{\mathcal C})$, and
$V^{\bar\pi}=\bar{\mathcal T}^{\bar\pi}_K V^{\bar\pi}$. Then
\begin{align}
...
This direct result is Bellman contraction on the actual finite-depth augmented evaluator. It requires no latent fixed point, recurrent contraction, Lipschitz head, projection, or slow-drift assumption. The augmented residual differs from the episodic residual on $(x,y,h)$ because both the policy and transition can depend on the carried latent. A learned instantiation would additionally need to establish the uniform augmented residual, exact augmented-state centering, and pointwise exact-mixture deployment.
...
Fix one frozen recurrent map $F$, one latent norm, and one bounded scalar
head $V_\psi$. For episodic state use
$z_h^{(0)}(x,y,h)=z_{\mathrm{init}}(x,y)$; for persistent state use the carried
pre-unroll latent explicitly, $z_h^{(0)}(x,y,z,h)=z$. For every depth $q\ge0$,
use the common construction
\[
...
\end{align}
If $V_\psi$ is uniformly $L_V$-Lipschitz in the common latent norm while $(x,y)$ is held fixed, then
\begin{align}
D_{h;n,m}
&\le L_V\sum_{q=n}^{m-1}
...
diagnostic and does not instantiate these theorems. Any learned instantiation
...
\textbf{Setup.}
Define the \emph{advantage function under the candidate policy $\pi_{\mathrm{cand}}$}:
\begin{equation}
...
\end{align*}
Rearranging gives~\eqref{eq:finite_depth_residual_certificate}. Lemma~\ref{lem:A_error_app} applied to the exact one-step centered estimator $\widehat A_{U_n}$ then yields~\eqref{eq:finite_depth_residual_adv}; Jensen's inequality and the uniform advantage-error bound give $\varepsilon_{A,\mathrm{cand}}\le\varepsilon_A$. Substituting that bound into Theorem~\ref{thm:monotone_with_error} gives~\eqref{eq:finite_depth_residual_cpi}.
\end{proof}
...
$\mathcal T_K^\pi:B(\mathcal C)\to B(\mathcal C)$. Assume
Assumption~\ref{ass:Lip} holds with $\mathcal Z_{\mathrm{inv}}=B_R$, the
fixed-point map $z^*$ is measurable, and the induced evaluator
$U_*(x,y)=V_\psi(z^*(x,y),x,y)$ belongs to $B(\mathcal C)$ with the common
absorbing boundary. Fix a measurable anchor map
$z_{\mathrm{anc}}:\XX\to B_R$ that depends only on the instance $x$, require
$B_{\mathrm{anc}}\in B(\mathcal C)$, and define
\begin{align*}
B_{\mathrm{anc}}(x,y)&:=V_\psi(z_{\mathrm{anc}}(x),x,y), &
...
\end{align*}
All suprema below range over the nonabsorbing states of $\mathcal C$.
Let
\[
D_{\mathrm{anc}}(R)
:=
...
L_V(R)
:=
\sup_{(x,y)}\sup_{\substack{z,z'\in B_R\\ z\neq z'}}
\frac{|V_\psi(z,x,y)-V_\psi(z',x,y)|}{\|z-z'\|_2}.
\]
Then
\begin{equation}
\varepsilon_{\mathrm{res}}^*
...
&=
|V_\psi(z^*(x,y),x,y)-V_\psi(z_{\mathrm{anc}}(x),x,y)| \\
&\le
