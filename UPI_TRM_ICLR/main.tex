...
The evidence has two distinct roles. First, an exactly solvable finite MDP numerically unit-tests the composed value, advantage, and CPI inequalities; it is not an empirical discovery. Second, experiments on hard $4\times4$ Sudoku evaluate implementations. The theory-aligned episodic run obtains $0\%$ success over 10 seeds. A persistent, distilled implementation obtains $57.4\%\pm12.2\%$ in a supplied UPI--TRM reevaluation artifact. The manuscript-recorded TRM+PPO result is $32.0\%\pm15.3\%$ at the same nominal outer-step count, but its per-seed run artifact is unavailable and the environment-interaction counts differ. We therefore claim neither equal-interaction superiority nor sample efficiency. A reported projection--clamping factorial has only aggregate provenance; we treat projection as a composite forward-pass and training-dynamics intervention and do not claim that clamping controls the global contraction modulus.
...
\item A numerical finite-MDP unit test and a $4\times4$ Sudoku study that reports the theory-aligned $0\%$ result, the persistent implementation result under unequal interactions, and aggregate projection--clamping contrasts with their provenance limitations.
\end{itemize}
...
\begin{table}[t]
\caption{No-mask hard 4$\times$4 results on 6--8 empties ($T{=}16$, mean $\pm$ std across seeds, nominal 20k outer-step protocol; persistent-$z$). A per-seed reevaluation artifact supports the UPI--TRM row. Baseline rows are manuscript-recorded aggregates whose per-seed run artifacts are unavailable. The protocol does not equalize environment interactions across rows.}
...
\midrule
\multicolumn{6}{@{}l}{\emph{Architecture-matched, manuscript-recorded aggregates (same TRM backbone):}} \\
...
At the nominal endpoint, the ten manuscript-recorded seed values imply a $+25.4$pp UPI--TRM/TRM+PPO gap (Welch $t\!\approx\!4.10$, $p\!\approx\!7.3\!\times\!10^{-4}$, 95\% CI $[12.4,38.5]$pp). The arithmetic is reproducible, but the PPO per-seed run artifact is unavailable. This joint, unequal-interaction implementation contrast isolates no component and implies no efficiency advantage (Appendix~\ref{app:headline_significance}).
...
The main result is a fixed-snapshot decomposition: a finite-depth recurrent evaluator can be compared with any deeper finite reference through a recurrent-path term and a reference-depth Bellman residual. Contraction supplies one optional geometric specialization rather than a model of arbitrary TRM computation. Persistent latents admit the same direct residual argument on a clock-complete augmented MDP, but the headline implementation does not provide the uniform augmented residual, exact centering, or exact-mixture deployment required to instantiate the CPI bound.

The present empirical evidence is limited to $4\times4$ Sudoku. The theory-aligned episodic configuration fails under the tested budget, the UPI--TRM/PPO comparison is not interaction matched, projection has a composite forward-pass and training-dynamics effect, and the clamping-enabled contrast is inconclusive and lacks per-seed provenance. The two decision-relevant next tests are an interaction-matched comparison and a one-factor-at-a-time bridge between the persistent and episodic implementations.
...
Persistent / PPO results & Nominal implementation protocol & UPI per-seed reevaluation supplied; PPO per-seed run artifact unavailable; interactions unequal \\
...
\begin{table}[t]
\caption{Representative local Lipschitz proxy $\hat L_z$ for the inner latent update map under different contraction-oriented components (4$\times$4 Sudoku batch; $\varepsilon=10^{-3}$). Power-iteration spectral normalization (SN) can cause the proxy to explode in our inner recursion, whereas operator-norm clamping (CLAMP) remains stable. These local values are not global certificates.}
...
\subsection{Reported Projection $\times$ Clamping Aggregates on Hard 4$\times$4}
\label{sec:controlled_2x2_main}

The manuscript records aggregate cells from a $2\times2$ factorial on the same no-mask hard 4$\times$4 suite. The per-seed factorial artifact and statistical test script are unavailable, so the table supports only arithmetic comparisons among the reported aggregates.

\begin{table}[t]
\caption{Manuscript-reported no-mask hard 4$\times$4 2$\times$2 aggregates (10 seeds per cell). The projection contrast is larger; the clamping-enabled contrast is small and inconclusive. Per-seed artifacts are unavailable.}
...
Historical hard-suite comparisons mixed projection settings and therefore confounded the clamping intervention with clipping. The recorded $2\times2$ aggregate uses the same no-mask hard 4$\times$4 Sudoku protocol (6--8 empties, 20k steps, 10 seeds), but the submitted evidence lacks per-seed provenance.
...
The significance calculation in Section~\ref{sec:harder_puzzles} uses the ten seed-level success rates recorded in the manuscript for the nominal 20k outer-step hard-suite protocol. For UPI--TRM, the rates are also present in the submitted reevaluation artifact:
\[
...
\]
with mean $32.0\%$ and sample standard deviation $15.3\%$. Applying Welch's $t$-test to these recorded values gives $t\approx 4.10$, $\mathrm{df}\approx 17.1$, two-sided $p\approx 7.3\times 10^{-4}$, and a 95\% confidence interval of $[12.4, 38.5]$ percentage points for the mean difference. These calculations establish arithmetic consistency only: the submitted evidence does not contain the PPO per-seed run artifact or an independent resampling script.

\subsection{Theory-Aligned Episodic-$z$ Rerun on the Hard Suite}
\label{app:episodic_z_hard_suite_rerun}

We ran a protocol-specified, theory-aligned rerun on the same no-mask hard 4$\times$4 Sudoku suite used in Table~\ref{tab:hard_suite_anchor} (6--8 empties, $T{=}16$, 20k environment steps, seeds 41--50). The supplied files do not establish that the protocol was timestamped before execution. The rerun starts from the persistent-$z$ hard-suite configuration and jointly changes four factors: episodic-$z$ latents, exact baseline summation, exact $K$-step targets, and explicit conservative mixtures. The architecture-matched evaluation interface and the inherited persistent-$z$ hyperparameters ($\gamma{=}0.99$, $K{=}1$, $n{=}2$) are unchanged. Diagnostics are computed on a frozen 256-instance closure batch (seed 1729). The fixed test split of the canonical 6--8-empty dataset contains only 99 viable puzzles, so the closure batch is sampled from the \texttt{train/} split with an independent RNG path; this deviation affects only the diagnostic batch, not the task-evaluation pool or the training seeds.

The paper tables below curate the main quantities. The submitted supplement includes \texttt{EPISODIC\_Z\_RUN\_LOG.md}, \texttt{episodic\_diagnostics\_summary.json}, \texttt{cpi\_penalty\_grid.json}, and \texttt{episodic\_per\_seed\_diagnostics.jsonl}.
...
\begin{table}[t]
\caption{Protocol-specified theory-aligned episodic-$z$ rerun at 20k on the no-mask hard 4$\times$4 suite. Means $\pm$ sample std over 10 seeds unless noted.}
