...
The evidence has two distinct roles. First, an exactly solvable finite MDP numerically unit-tests the composed value, advantage, and CPI inequalities; it is not an empirical discovery. Second, retained hard-$4\times4$ Sudoku artifacts record historical implementations. The episodic run reports $0\%$ success over 10 seeds, but its endpoint evaluator scored only the old-policy component. The persistent artifact cycles evaluation over training records and deploys parameter interpolation rather than distillation or the exact pointwise mixture. Per-seed logs retain the manuscript-recorded TRM+PPO and projection--clamping arithmetic, but the checkpoints, hard-suite dataset, ordered evaluation population, and exact training revision are absent; interaction counts also differ. We attach no comparative or inferential interpretation to these records; their numerical details are confined to Appendix~\ref{app:headline_significance}.
...
Corollary~\ref{cor:cpi_dials_main} is the optional fixed-point specialization of the finite-reference penalty $2\alpha\gamma B_{n,m}/(1-\gamma)$. It makes the one-step dependence on $(L_z,n,K,\alpha)$ explicit. For $n\ge1$, its recurrent term vanishes as $L_z\to0$; together with $\varepsilon_{\mathrm{res}}^*\to0$, this recovers the standard CPI expression. At $n=0$, the displayed recurrent term instead tends to $L_VC_z$ and no such claim is made. Two constants in~\eqref{eq:main_bound_dials} are observed rather than tuned: $L_V$ is only locally estimated by finite differences, and the current clamping mechanism produces little range in one projection-active proxy $\hat L_z^{\mathrm{post}}$ ($\approx0.22$--$0.24$). On the hard suite, we therefore treat the $L_z$-dependent terms as monitoring quantities.
...
All task records in this paper are on $4\times4$ Sudoku. The only theory-oriented task endpoint retained in the main text is the negative episodic old-policy result at $0\%$. The persistent, baseline, and projection $\times$ clamping values are historical execution records confined to the appendix. None is a held-out method comparison or validation of the fixed-snapshot theorem; Appendix Table~\ref{tab:claim_status} maps each claim to its conditions, provenance, and status.
...
The historical persistent implementation used parameter interpolation of the edit-policy head, evaluated on training records, dropped carried latents from replay updates, and omitted the remaining-edit clock from recorded state. Its uniform augmented residual and centering are unmeasured. Appendix Table~\ref{tab:hard_suite_anchor} preserves the numerical record for auditability; it is not a held-out result, a comparative experiment, or verification of Corollary~\ref{cor:persistent_cpi_augmented} or Theorem~\ref{thm:monotone_with_error}.

\subsection{Projection and clamping ablation}
\label{sec:projection_contraction_ablation}

The implementation repository retains per-seed logs that reproduce the $2\times2$ projection $\times$ clamping cell arithmetic in Appendix Table~\ref{tab:controlled_2x2_main}. The original statistical-test script, checkpoints, dataset, and ordered evaluation records are absent, so the source-reported test and a causal mechanism interpretation are \emph{not verifiable from supplied evidence}. Projection has the larger historical composite contrast, but that contrast mixes clipping and training dynamics; the clamping-enabled contrast is small. The theorem's contraction condition remains a conditional error-control mechanism, not a demonstrated performance dial.
...
PPO/A2C/DQN collect 64/32/4 transitions per nominal step. UPI--TRM is absent from the incomplete common-budget attempt; its retained baseline logs do not form a complete comparison. There is no matched UPI--TRM--PPO result (Appendix~\ref{app:equalized_baselines}).
...
The retained empirical records are limited to $4\times4$ Sudoku and do not provide a held-out method comparison. The episodic endpoint did not score its configured mixture, the persistent record is in-sample, the UPI--TRM/PPO protocols are not interaction matched, and projection has a composite forward-pass and training-dynamics effect. Per-seed factorial arithmetic is retained, but checkpoints, the hard-suite dataset, ordered per-instance outcomes, and the original statistical-test script are absent. Three decision-relevant requirements remain: recover or replace the missing persistent checkpoints and run augmented-state diagnostics, run a one-factor-at-a-time bridge, and run a common-pool interaction-matched UPI--TRM/PPO comparison. The historical checkpoint diagnostic is currently blocked by the missing checkpoints and data.
...
Persistent / PPO records & Common held-out pool and matched interactions & Per-seed arithmetic is retained; comparison is \emph{not verifiable from supplied evidence} because the pool and interactions differ \\
Projection / clamping & Complete historical provenance and a controlled achieved modulus & Per-seed arithmetic is retained; mechanism claim is \emph{not verifiable from supplied evidence} \\
Finite-MDP inequalities & Exact finite-state calculation & Working-tree CSV has 101 rows with zero numerical violations; archive supply is \emph{not verifiable from supplied evidence} \\
\bottomrule
\end{tabularx}
\end{table}
...
The following studies probe mechanisms and implementation behavior. They are not theorem validation and are not used as confirmatory hard-suite evidence unless explicitly stated. Their retained aggregates can be re-read, but checkpoint identity, raw B0/B1 batch identity, and fresh execution are \emph{not verifiable from supplied evidence}.

\paragraph{Finite-difference $L_V$ diagnostic for the main comparison.}
The retained Exp1 refreeze summary reports a local finite-difference proxy for $L_V$ on frozen B0 states with episodic-$z$, $n_{\mathrm{eval}}{=}8$, and seeds 41--50. Table~\ref{tab:exp1_value_head_lipschitz} gives the seed-aggregated values. The smallest perturbation scale $\epsilon=10^{-5}$ is noisier, but over $\epsilon\in\{3\times 10^{-5}, 10^{-4}, 3\times 10^{-4}, 10^{-3}\}$ both conditions remain in the same-order $0.085$--$0.111$ band. This is a retained local diagnostic, not a certified global Lipschitz bound or a fresh reproduction.
...
The retained $2\times 2$ diagnostic separates $z\!\to\!z$ clamping from value-head normalization, which also affects $\mathrm{Var}(V)$ but is architecturally separate.
...
The retained summary includes pre-projection latent norms $\|z^{(n)}_{\text{pre}}\|$ and total variance $\mathrm{Var}(z^{(n)}_{\text{pre}})$, pairwise cosine similarity, value variance $\mathrm{Var}(V)$, and post-projection norms $\|z^{(n)}_{\text{post}}\|=\|\Pi_R(z^{(n)}_{\text{pre}})\|$.
...
The retained fixed-B0 summary uses $(n_{\mathrm{train}}, n_{\mathrm{eval}})=(2,8)$ from Section~\ref{sec:isolation_study}, radii $\{\text{proj.\ off}, 5, 10, 20, 30, 32, 34, 40, 100\}$, and seed-level means over seeds 41--50. It records endpoint separation and an inactive plateau at $R{\ge}40$; the source pre-projection norm at $R{=}10$ lies in a narrow $p_{05}$--$p_{95}$ band of about $30.6$--$33.2$. The retained finite-batch residual moves non-monotonically in that transition window. Because the checkpoints and raw evaluation batches are absent, this summary does not freshly reproduce the result or identify $\varepsilon_{\mathrm{res}}^*(R)$ end to end.
...
\caption{Retained finite-batch theory diagnostics.}
\end{subfigure}
...
The retained inference-time scaling summary (scales $\in \{1.0, 0.85, 0.70, 0.55\}$, \emph{proj.\ off}, value-head spectral normalization disabled) reports modest stability variation and task success near $6.7\%$, without a pronounced tradeoff in that batch.
...
The historical manuscript describes a harder suite with 6--8 empty cells and a recorded random-policy success rate of 0\%. The underlying dataset and random-policy run artifact are absent, so this rate is \emph{not verifiable from supplied evidence}.

\subsection{Reported Projection $\times$ Clamping Aggregates on Hard $4\times4$}
\label{sec:controlled_2x2_main}

The implementation repository retains 40 per-seed logs, four configurations, the launcher, and a parser for a $2\times2$ factorial on the manuscript-described no-mask hard $4\times4$ suite. They reproduce the four displayed cell means and sample standard deviations. The checkpoints, historical dataset, ordered evaluation records, per-instance outcomes, and original statistical-test script are absent, so any causal or contraction-mechanism claim is \emph{not verifiable from supplied evidence}.

\begin{table}[t]
\caption{Historical no-mask hard $4\times4$ $2\times2$ records (10 seeds per cell). Retained per-seed logs reproduce the cell arithmetic, but checkpoints, dataset and ordered-pool provenance, per-instance outcomes, and the original statistical-test script are absent.}
...
The historical projection contrast raises success from $36.2\%$ to $49.2\%$ (+13.0pp averaged over clamping status). It is a composite protocol contrast spanning the forward pass and training dynamics (changes to $f_\theta$, $\varepsilon_0$, and the optimizer trajectory), which~\eqref{eq:main_bound_proj} does not model. Averaging over projection status, the clamping-enabled contrast raises success from $41.6\%$ to $43.8\%$ (+2.2pp). The source-reported $p\approx0.6$ is \emph{not verifiable from supplied evidence} because its statistical script is absent; the retained arithmetic does not support a contraction or variance-reduction claim.
...
Historical hard-suite comparisons mixed projection settings and therefore confounded the clamping intervention with clipping. Retained per-seed logs reproduce the recorded $2\times2$ arithmetic for the manuscript-described no-mask hard $4\times4$ Sudoku protocol (6--8 empties, 20k steps, 10 seeds), but the dataset, ordered-pool hashes, checkpoints, and per-instance outcomes are absent. A causal mechanism interpretation is \emph{not verifiable from supplied evidence}.
...
Table~\ref{tab:hard_suite_anchor} reports the nominal 20k outer-step protocol. Because PPO, A2C, and DQN collect 64, 32, and 4 transitions per outer step, it does not equalize interactions. The source manuscript recorded a baseline-only common-budget attempt, but only partial logs remain and the UPI--TRM arm was never run. We therefore remove those incomplete numbers from active claims and retain only the artifact-status audit below.
...
\midrule
Source baseline-only audit & Absent & Partial / incomplete & Numerical rows excluded \\
...
\begin{table}[t]
\caption{Historical no-mask hard $4\times4$ execution records (mean $\pm$ sample std across seeds). The UPI artifact cycles 50 episodes over 32 training records also used for training. Architecture-matched rows are manuscript-recorded aggregates; external rows use a different budget unit and lack ordered-pool hashes. These rows are not a comparative experiment. The retained evaluator code defines ``Return'' as the undiscounted episode sum of emitted rewards, distinct from discounted $\eta(\pi)$; the exact producer revision for every historical row is \emph{not verifiable from supplied evidence}.}
\label{tab:hard_suite_anchor}
\centering
\scriptsize
\begin{tabular}{@{}lccccc@{}}
\toprule
Historical record & Recorded budget & Seeds & Success (\%) & Return & Invalid \\
\midrule
UPI--TRM (in-sample) & 20k outer updates & 10 & 57.4 $\pm$ 12.2 & -1.82 $\pm$ 2.66 & 0.000 $\pm$ 0.000 \\
\midrule
\multicolumn{6}{@{}l}{\emph{Architecture-matched manuscript records (same TRM backbone):}} \\
...
\midrule
\multicolumn{6}{@{}l}{\emph{External SB3 records (flat MLP policy; high invalid-action rates):}} \\
PPO/A2C/DQN/DQN($n{=}5$) & 20k env steps & 10 & 0.0 $\pm$ 0.0 & $\approx -17.9$ & 0.78--1.00 \\
\bottomrule
\end{tabular}
\end{table}

The following calculation preserves the ten seed-level success rates printed in the historical manuscript. The UPI--TRM reevaluation artifact cycles 50 episodes over 32 records from the training split that were also used during training, and it contains no ordered-pool hash. Ten retained PPO logs reproduce its aggregate, but the checkpoints, historical dataset, ordered pool, per-instance outcomes, and exact training revision are absent. A method comparison is therefore \emph{not verifiable from supplied evidence}. For UPI--TRM, the recorded rates are
\[
...
\nolinkurl{per_seed_diagnostics.jsonl}. Their presence in the submitted review
package is \emph{not verifiable from supplied evidence}; even if included, the
missing source logs and checkpoints prevent rerunning or independently
reconstructing the endpoint.
...
\begin{table}[t]
\caption{Retained theory-oriented episodic-$z$ records at 20k on the manuscript-described no-mask hard $4\times4$ suite. The task endpoint evaluates only the old policy. Means $\pm$ sample std over 10 seeds unless noted. The retained evaluator code defines ``Return'' as the undiscounted episode sum of shaped rewards and ``Score'' as the final checker score, but the exact historical producer revision is \emph{not verifiable from supplied evidence}. Return is distinct from discounted $\eta(\pi)$.}
...
Historical branch label & D (legacy bookkeeping only) \\
\midrule
...
\STATE Update $(\theta,\psi)$ by minimizing $\mathcal{L}_{\mathrm{val}}$, then freeze the evaluator snapshot $F_n$
...
The frozen CPI comparison begins only after the value update and uses the same fixed recurrent map $F_n$ for the current and candidate action kernels through the mixture step. Any subsequent recurrent-parameter update starts a new snapshot and is outside Theorem~\ref{thm:monotone_with_error}. Source audit found that the historical implementation did not retain the augmented replay state needed for this comparison and used parameter interpolation for deployment. The repaired code takes an explicit post-value-update evaluator snapshot, but the missing hard-suite data and checkpoints prevent a corrected rerun of the recorded experiments.
