...
real functions on $\mathcal C$. All displayed norms are ordinary sup norms
...
\end{equation}
with $\varepsilon_0:=\|B_0-\mathcal{T}^\pi_K B_0\|_{\infty,\pi,\pi_{\mathrm{cand}}}^{\mathrm{adv}}$. Projection thus enters both the architectural term (through $L_V(R)R$) and the truncation term (through $L_z^{\mathrm{sat}}(R)$ and $C_z\le 2R$). The retained experiments do not report these quantities for the successful persistent implementation. Future finite-batch measurements would be diagnostics, not certificates of the uniform $\rho_R$ and Lipschitz assumptions. Likewise, a batch $U_n$ Bellman residual is only a proxy: the global sup-norm $U_n$ residual enters the upper bound on $\varepsilon_{\mathrm{res}}^*$ together with the truncation gap (Remark~\ref{rem:residual_bridge}).
...
implementation is not verifiable from supplied evidence.
...
are therefore not verifiable from supplied evidence. Historical projection/clamping
...
regeneration are not verifiable from supplied evidence. This record is
...
not provide a held-out method comparison. The episodic endpoint does not
establish that it scored its configured mixture, the persistent record is
in-sample, and the historical
protocols are not interaction matched. A registered augmented-state diagnostic
runner and an immutable corpus with input- and record-disjoint
train/validation/test splits now exist. Completed grids are not disjoint: the
corpus covers 283 of the 288 valid $4\times4$ completions, and 494 of 512 test
inputs share their completion with a training input. The test split therefore
measures constraint satisfaction on unseen givens, not generalization to unseen
solution grids. A compatible successful persistent checkpoint is still
missing. The bridge and common-pool matched comparison are also missing. Until
...
Persistent learned-model record & Reconstructible checkpoint, held-out population, and exact policy identity & Task result and diagnostics are not verifiable from supplied evidence \\
Bridge and matched PPO comparison & Registered one-factor bridge and common-pool equal interactions & missing experiment \\
Projection mechanism & Train/evaluation cross-design and a certified modulus for contraction claims & Missing experiment; historical mechanism claim is not verifiable from supplied evidence \\
...
Figure~\ref{fig:finite_mdp_certificate_main} shows two checks. Left: for $\gamma{=}0.9$, $K{=}3$, $\sigma_\xi{=}0.05$, the value error decays with $n$ and worsens as $L_z\to 1$, staying below the contractive specialization of Theorem~\ref{thm:finite_reference_depth}. Right: on an evaluator-based candidate slice ($L_z{=}0.2$, $n{=}16$, $\sigma_\xi{=}0.1$, $\beta{=}1$), the surrogate gap $\widehat L_\pi(\pi_\alpha)-\eta(\pi_\alpha)$ is positive for every tested $\alpha>0$ and stays below both the decomposition-based CPI penalty and the tighter $\varepsilon_{A,\mathrm{cand}}$ penalty for every tested $\alpha$ (log scale; the bound is conservative). At $\alpha=0$, the gap and both penalties are zero. Corollary~\ref{cor:finite_depth_residual_certificate} is directly checkable here because $\|U_n-\mathcal{T}^\pi_K U_n\|_\infty$ is computable. The script checks 80 value/decomposition rows ($4$ $L_z$ values by $20$ depths) and 21 $\alpha$ rows on this selected CPI slice, with zero violations; it does not constitute a full Cartesian sweep over $(L_z,n,K,\sigma_\xi,\alpha)$.

The working tree contains \texttt{finite\_mdp\_certificate.py}, a \texttt{finite\_mdp\_summary.csv} with 80 \texttt{value\_curve} and 21 \texttt{cpi\_curve} rows whose certificate flags are true, and \texttt{finite\_mdp\_certificate\_validation.log}. Their presence in the submitted review package is not verifiable from supplied evidence.

\begin{figure}[t]
\centering
\begin{subfigure}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth]{results/finite_mdp_certificate_validation/fig_value_decomposition.png}
\caption{Value-error decomposition.}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\textwidth}
\centering
\includegraphics[width=\linewidth]{results/finite_mdp_certificate_validation/fig_cpi_certificate.png}
...
$\|U_n-\mathcal T_K^\pi U_n\|$ & Remark~\ref{rem:residual_bridge} & Monte Carlo $K$-step operator estimate and standard error on retained states; distinct from realized-path TD error \\
$\bar\varepsilon_{\mathrm{res},n}$ & Cor.~\ref{cor:persistent_cpi_augmented} & Exact one-step and Monte Carlo $K$-step augmented-state residual diagnostics at depths $n$ and $m$ \\
$\varepsilon_{\mathrm{cent}}$ & Thm.~\ref{thm:monotone_with_error} & Reconstructed exact action sum after configured recentering; not saved training-time evidence \\
$D_{A,\mathrm{cand}}^{n,m}$ & Finite-reference diagnostic & Batch $|\E_{\pi_{\mathrm{cand}}}[\widehat A_n-\widehat A_m]|$; not the theorem's uniform $\varepsilon_{A,\mathrm{cand}}$ \\
TV / KL deployment gap & Thm.~\ref{thm:deployment_perturbation} & Exact mixture versus production distribution callback on retained states; not a uniform closure bound \\
\bottomrule
\end{tabularx}
\end{table}
...
therefore not verifiable from supplied evidence. No
