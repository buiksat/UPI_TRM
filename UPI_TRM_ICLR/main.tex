...
The evidence has two distinct roles. First, an exactly solvable finite MDP numerically unit-tests the composed value, advantage, and CPI inequalities; it is not an empirical discovery. Second, retained hard-$4\times4$ Sudoku artifacts document historical implementations. The episodic run records $0\%$ success over 10 seeds, but its endpoint evaluator scored only the old-policy component. The persistent evaluator cycled over training records and deployed parameter interpolation rather than the exact pointwise mixture. The checkpoints, hard-suite dataset, ordered evaluation population, and exact training revision are absent, and interaction counts differ. We therefore exclude the old performance and ablation tables and attach no comparative or inferential interpretation to those records.
...
The theory-oriented episodic configuration reinitialized the latent and used
exact baseline summation, exact $K$-step targets, and explicit-mixture
collection. Its endpoint evaluator nevertheless scored only
\texttt{policy\_model\_old}, and task evaluation reused a materialized
32-record training pool. The retained aggregate records $0.0\%$ success at
the nominal 20k-environment-step endpoint over seeds 41--50. This is an
in-sample old-policy endpoint, not an evaluation of the exact mixture and not
a controlled persistent-versus-episodic comparison. Regeneration from the
original checkpoints and per-instance outcomes is
\emph{not verifiable from supplied evidence}.
...
TV / KL deployment gap & App.~\ref{app:implementation} & Exact mixture versus deployed-policy discrepancy on declared states \\
\bottomrule
\end{tabularx}
\end{table}
...
nominal 20k outer-step protocol gives different interaction budgets because
each baseline collects a different number of transitions per outer step. No
interaction-equalized UPI--TRM/PPO comparison has been run.
